...
parser.add_argument("-b",action="store_true", help="vectorized solver without subsea geometry and damping. If -s and -b Scalar solver without damping and subsea geometry")
...
#--------Initial conditions------------
def initial(x,y,x_0,y_0,sigma_x,sigma_y):
	"""
...
x0 = random.randint((Lx/2)-(Lx/4),(Lx/2)+(Lx/4))
y0 = random.randint((Ly/2)-(Ly/4),(Ly/2)+(Ly/4))
...
def solve_scalar_simple(u0,u1,uny):
	...
	for i in xrange(T):	
		...
		print i
	return None
def solve_vectorized(u0,u1,uny,q,f,T,n):
	...
	for i in xrange(T):
		...
		if (i)%5 == 0:
			...
			for o in xrange(1,n-1):
				for p in xrange(1,n-1):
					f[o,p] = 5*initial(X[o,p],Y[o,p],x0,y0,sigma_x,sigma_y);
		...
		if i%5 == 0:
			savetxt('texttmp%.4d.txt'%counter,u0)
			counter += 1
	return None
#--------Working loop---------------------
# u1 = up
...
if args.s and not args.b:
	...
	solve_vectorized(u0,u1,uny,q,f,T,n)
...
if args.movie and args.rm:
	...
	if failure:
		print 'Execution of "%s failed\n' %cmd
	text_files = glob('texttmp*.txt')
	img_files = glob("wtmp*.png")
